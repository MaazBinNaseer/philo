...
/* assign_forks:
*	 Even-numbered philosophers get their fork order switched. 
*   This is because the order in which
...
t_table	*init_table(int ac, char **av, int i)
{
	...
	if (!table)
		return (error_null(STR_ERR_MALLOC, NULL, 0));
	table->nb_philos = ft_small_atoi(av[i++]);
	table->time_to_die = ft_small_atoi(av[i++]);
	table->time_to_eat = ft_small_atoi(av[i++]);
	table->time_to_sleep = ft_small_atoi(av[i++]);
	table->must_eat_count = -1;
	if (ac - 1 == 5)
		table->must_eat_count = ft_small_atoi(av[i]);
