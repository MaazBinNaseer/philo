...
/* start_simulation:
*	Launches the simulation by creating a kill checker thread as well as
...
static bool	start_simulation(t_table *table)
{
	...
	{
		if (pthread_create(&table->it_is_i_death, NULL,
				&kill_checker, table) != 0)
			return (error_failure(STR_ERR_THREAD, NULL, table));
	}
	return (true);
}
...
static void	stop_simulation(t_table	*table)
{
	...
	if (table->nb_philos > 1)
		pthread_join(table->it_is_i_death, NULL);
